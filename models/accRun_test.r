accRun <- function(featureData,
  ...
  if(Dummies){
    ...
    testData <- merge(testData, sealCharacteristic[,1:6], by = "SealName")
    ...
    testData$harsness <- factor(testData$harsness)  
  }
  else{
    # browser()
  ...
  if(SAVE){
    ...
    fName <- paste0(Model, " Test, ","Hz = ", Hz,", epochs = ", epochs,".RData")

    cat("##### saving", fName, "\n")
    
    #mainDir <- getwd()
    if(codeTest){
      subDir <- "I:/Behaviour accelerometry/behaviour_accelerometry-master/model_outputs/cross-validation/"
    }
    else{
      subDir <- "I:/Behaviour accelerometry/behaviour_accelerometry-master/model_outputs/cross-validation/"
    }
